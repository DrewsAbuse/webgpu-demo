...
@group(0) @binding(3) var<uniform> frameLine: LineUniforms;
...
@vertex fn vsIndexedU32(@builtin(vertex_index) vNdx: u32) -> VSOut {
  ...
  let pNdx = index * frameLine.stride;
...
fn edgeFactor(bary: vec3f) -> f32 {
  let d = fwidth(bary);
  let a3 = smoothstep(vec3f(0.0), d * frameLine.thickness, bary);
